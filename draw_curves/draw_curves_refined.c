...
// //間違えて５つ目の点を消すプログラムを書いてしまったので残しておきます。
...
//                 list_node *node = NULL;
//                 node->x = x;
//                 node->y = WINDOW_HEIGHT - y - 1;
//                 list.root = node;
//             } else {
//                 add_node(&list, x, WINDOW_HEIGHT - y - 1);
//             }
//             n_points++;
//         } else {
//             // リストから要素を削除 (先頭の要素を削除)
...
//         int i = 0;
//         while(iter != NULL){
//             mat_elem(points, i, 0) = iter->x;
//             mat_elem(points, i, 1) = iter->y;
//             iter = iter->next;
//             i++;
//         }
//         glutPostRedisplay();
...
void mouse(int button, int state, int x, int y) {
    if (button == GLUT_LEFT_BUTTON && state == GLUT_DOWN) {
        ...
        while(iter != NULL){
            double dx = iter->x - x;
            double dy = iter->y - (WINDOW_HEIGHT - y - 1);
            if (dx * dx + dy * dy <= 64) {
                list_node *iter_temp = iter;
                iter = iter->next;
                remove_node(&list, iter_temp);
                n_points--;
            } else {
                iter = iter->next;
            }
